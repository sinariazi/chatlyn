...
import prisma, { type Message, type Conversation } from "@/lib/db"
import {
  trackRuleTriggered,
  trackRuleExecuted,
            ...
            if (reply) {
              await prisma.message.create({
                data: {
                  ...
                  content: reply,
                  contentType: "TEXT",
                  metadata: { autoGenerated: true, ruleId: rule.id } as Record<string, unknown>,
                },
              })
            ...
            if (action.template) {
              ...
              await prisma.message.create({
                data: {
                  ...
                  content: reply,
                  contentType: "TEXT",
                  metadata: { autoGenerated: true, ruleId: rule.id, template: true } as Record<string, unknown>,
                },
              })
              generatedReply = reply
